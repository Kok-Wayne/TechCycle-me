...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($check_result->num_rows > 0) {
        header("Location: ../UI_screens/register.php?error=username_exists");
        ...
        if ($email_result->num_rows > 0) {
            header("Location: ../UI_screens/register.php?error=email_exists");
            $email_stmt->close();
            $conn->close();
            exit();
            ...
            if ($phone_result->num_rows > 0) {
                header("Location: ../UI_screens/register.php?error=phone_exists");
                $phone_stmt->close();
                $conn->close();
                exit();
            }
            ...
            $insert_sql = "INSERT INTO users (username, email, password, phoneNumber) VALUES (?, ?, ?, ?)";
