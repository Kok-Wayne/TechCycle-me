<?php
    ...
    $completedGroups = [
        ...
        'This Month'=>[],'Last Month'=>[],'This Year'=>[],'Last Year'=>[],'Others'=>[],
    ];
    $now = new DateTime(); $today = new DateTime('today'); $yesterday = new DateTime('yesterday');

    foreach ($allCompleted as $row) {
        $dateStr = $row['collectedAt'] ?? null;
        if (!$dateStr) { $completedGroups['Others'][] = $row; continue; }
        $date = new DateTime($dateStr);
        if      ($date >= $today)                                                                    $completedGroups['Today'][]      = $row;
        elseif  ($date >= $yesterday)                                                                $completedGroups['Yesterday'][]  = $row;
        elseif  ($date >= new DateTime('monday this week'))                                          $completedGroups['This Week'][]  = $row;
        elseif  ($date >= new DateTime('monday last week'))                                          $completedGroups['Last Week'][]  = $row;
        elseif  ($date->format('Y-m') === $now->format('Y-m'))                                      $completedGroups['This Month'][] = $row;
        elseif  ($date->format('Y-m') === (new DateTime('first day of last month'))->format('Y-m')) $completedGroups['Last Month'][] = $row;
        elseif  ($date->format('Y') === $now->format('Y'))                                          $completedGroups['This Year'][]  = $row;
        elseif  ($date->format('Y') === (string)((int)$now->format('Y') - 1))                      $completedGroups['Last Year'][]  = $row;
        else                                                                                         $completedGroups['Others'][]     = $row;
    }
    ...
    <main id="content">
        <div class="pickups-wrapper">
            ...
            <div class="pickup-tab-container">
                ...
                <button class="pickup-tab" onclick="switchTab('completed', this)">Completed</button>
            </div>
            ...
            <div id="panel-pending" class="pickup-panel active">
                <div class="pickup-search-wrap">
                    ...
                    <input type="text" id="search-pending" class="pickup-search" placeholder="Search product name or location..." oninput="filterPending()">
                </div>
                <div id="list-pending">
                    ...
                    <?php else: ?>
                        ...
                        <a class="pickup-item" href="workerProductDetail.php?id=<?= $row['productID'] ?>"
                        ...
                        data-location="<?= strtolower(htmlspecialchars($row['centreName'] ?? '')) ?>">
                            <div class="pickup-item-info">
                                <div class="pickup-item-name"><?= htmlspecialchars($row['productName']) ?></div>
                            ...
                            <?php if (!empty($row['productImage'])): ?>
                                <img class="pickup-item-img" src="/TechCycle/<?= htmlspecialchars($row['productImage']) ?>" alt="<?= htmlspecialchars($row['productName']) ?>">
                            <?php else: ?>
                                <div class="pickup-item-placeholder">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                </div>
                            <?php endif; ?>
                        </a>
                        <?php endforeach; ?>
                        <div class="pickup-no-results" id="no-results-pending">No results match your search.</div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div id="panel-completed" class="pickup-panel">
                <div class="pickup-search-wrap">
                    ...
                    <input type="text" id="search-completed" class="pickup-search" placeholder="Search product name or location..." oninput="filterCompleted()">
                </div>
                <div id="list-completed">
                    ...
                    <?php else: ?>
                        <?php foreach ($completedGroups as $groupName => $items): ?>
                            ...
                            <div class="pickup-group">
                                <div class="pickup-group-header" onclick="toggleGroup(this)">
                                    ...
                                    <span><?= $groupName ?></span>
                                    <span class="pickup-group-count"><?= count($items) ?> item<?= count($items) !== 1 ? 's' : '' ?></span>
                                </div>
                        ...
                        <?php endforeach; ?>
                        <div class="pickup-no-results" id="no-results-completed">No results match your search.</div>
                    <?php endif; ?>
                </div>
            </div>
        </div>
...
<script>
    ...
    function filterCompleted() {
        ...
        groups.forEach(function(group) {
            ...
            if (q && gv > 0) { body.style.display = 'block'; header.classList.add('open'); }
            else if (!q)     { body.style.display = 'none';  header.classList.remove('open'); }
